...
| 28 | CRIT | core/awg_manager.py | AWG не поднимался: `amneziawg-go` ДЕМОНИЗИРУЕТСЯ (родитель выходит rc==0, демон создаёт сокет позже), а цикл ломался на выходе родителя → код находил живой демон и **убивал его SIGKILL**. Проявлялось как «awg setconf: Protocol not supported» на Keenetic. Диагностика с реального роутера подтвердила | ✅ fixed (rc==0=демонизация: ждём сокет; stderr в файл вместо блокирующегося pipe; убран ошибочный kernel-фолбэк) |
