...
> **Статус: архив.** Ветка `Development` слита в `main` и удалена, работа по
> этому плану завершена — найденные тогда дефекты и их фиксы описаны ниже и в
> `CHANGELOG.md`. Цифры в документе (число тестов, роутов, файлов в дифе) —
> снимок на момент того мержа, они НЕ отражают текущее состояние репозитория:
> актуальные счётчики берите из прогона (`python3 -m unittest discover -s tests`)
> и из `CoderManual.md`. Документ оставлен как запись метода: чек-лист
> «что и в каком порядке проверять перед крупным мержем» переиспользуем.
