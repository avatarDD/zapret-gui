...
| 28 | CRIT | core/awg_manager.py | AWG не поднимался на Keenetic: проверка готовности демона шла по хардкод-пути `/var/run/wireguard/<if>.sock`, которого там НЕТ (сокет в другом месте), + amneziawg-go работает в FOREGROUND (proc.poll()==None) → таймаут → код убивал живой демон. Проявлялось как «Protocol not supported» / «превышено время ожидания». Диагностика с 2 реальных роутеров | ✅ fixed (готовность через `awg show <iface>` rc==0 — не зависит от пути сокета и от foreground/демонизации; stderr в файл; убран ошибочный kernel-фолбэк) |
