...
- [x] **Авто-переподключение туннеля по качеству связи** —
      `core/awg_watchdog`: активная проба через туннель (SO_BINDTODEVICE)
      + handshake-age, рестарт при деградации; UI-галка «Переподключать
      при плохом соединении» на дашборде AWG. Эскалация awg→nfqws2 —
      через единый слой (monitor+failover).
- [x] **Установка не в /tmp на OpenWrt** (issue #98) —
      `binary_installer.workbase()` выбирает постоянный носитель;
      настройка `install.tmpdir` / env `ZAPRET_GUI_TMPDIR`.
- [x] **Понятный порядок меню + древовидность + подсказки «?»** —
      реорганизованы группы, разъяснены «Маршрутизация» vs «AWG-правила»
      и «Списки маршрутизации» vs «Домены/IP-списки».
- [x] **catalog: фантомное «обновление стратегий»** — сравнение по
      subpath каталогов, а не HEAD ветки.
- [ ] **Полевое тестирование** на железе: OpenWrt 22.03+ (nftset
      backend, /tmp→флешка) и KeenOS 4.x — код есть, не проверено на
      устройствах.
