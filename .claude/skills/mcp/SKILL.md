...
Форма записи не спасает: getopt_long nfqws2 принимает опцию с
обязательным значением и двумя токенами (`--pidfile /путь`), а стратегия
из текста режется по пробелам. Поэтому значение вырезается вместе с
опцией (`strategy_lint.TAKES_VALUE`, линтер называет его отдельной
находкой), а `--hostlist-auto <путь>` проходит ту же проверку каталога,
что и `--hostlist-auto=`.
