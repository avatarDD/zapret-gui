...
- Ловит **только**: кривой парсинг опций CLI и отсутствующие файлы
  (`--blob`/`--lua-init`/`--hostlist`/…). **Lua НЕ загружается и НЕ
  исполняется** (эталон `docs/manual.md`: «Lua не проверяется») → ни синтаксис
  lua-скриптов, ни наличие вызываемых `--lua-desync` функций dry-run НЕ
  проверяет. От вызова незагруженной функции (тихий 0%) защищает статический
  инвариант `tests/test_nfqws_lua_map.py`, а не dry-run.
- API:
