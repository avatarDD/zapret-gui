...
**Актуальный релиз — v1.0.4** (2026-07-31). Master после него (2026-08-02) —
только правки документации, кода не трогали. Предыдущий ориентир этого
справочника был v0.9.5.2 (2026-04-30).
...
2. ⚠️ **Ловушка `_protected_core_lua`.** `core/asset_importer.py` защищает
   ...
   защиты нет — bundle затирает upstream-копию. Поэтому апстримные фиксы
   «внутри compat» нужно переносить в `import/lua/` руками, иначе на свежем
   релизе мы откатываем пользователю его же lua.
   Известный дрейф на сейчас: наши `zapret-lib.lua` / `zapret-antidpi.lua`
   соответствуют уровню **1.0.3** — в 1.0.4 `desync_timer_name()` стал
   включать `desync.func_instance`, а в `send` убрали ручной префикс
   `"send_"`. Функционально это только уникальность имён таймеров, но дрейф
   реальный.
...
перезаписывает upstream-lua, см. §0.2.
