...
### Наши расширения (`import/lua/`, наезжают на `lua_path` при `lua_manager.sync()`)
...
2. **conntrack ограничитель**: первые N пакетов через `--connbytes 1:N
   ...
   экономия CPU. Параметры:
   - TCP: `nfqws.tcp_packets_out` / `_in` (см. `config_manager`).
   - UDP: `nfqws.udp_packets_out` / `_in`.
...
это поведение зашито в `firewall.py` (через детект Keenetic).
...
| `lua/*.lua` | `zapret.lua_path` | `zapret-lib.lua`, `zapret-antidpi.lua`, … + наши `import/lua/*.lua` после `lua_manager.sync()`. |
