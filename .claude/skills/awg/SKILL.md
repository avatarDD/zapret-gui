...
### 2.1.1 Поля тайминга и защиты заголовка (tools v3.0.20260730)

Свежий релиз amneziawg-tools добавил в `[Interface]` семь ключей, которых в
предыдущем (`v1.0.20260618-2`) не было:

| Ключ | Назначение |
|---|---|
| `HeaderProtectionKey` | Ключ защиты заголовка (шифрование header'а поверх обфускации) |
| `ContentPaddingAddition` | Дополнительный паддинг содержимого |
| `RekeyAfterTime`, `RekeyTimeout`, `RejectAfterTime` | Тайминги переустановления ключей |
| `KeepaliveTimeout` | Таймаут keepalive |
| `MaxHandshakeAttempts` | Предел попыток handshake |

Мы их пока **не генерируем и не валидируем** — в `WG_INTERFACE_FIELDS` их
нет. Практическое следствие: импортированный `.conf` с этими ключами наш
`validate()` пометит как неизвестные поля. Если такие профили начнут
встречаться — добавлять в `WG_INTERFACE_FIELDS` (они, в отличие от `J1..J3`,
парсером tools **поддерживаются**, так что в setconf уходить могут).
