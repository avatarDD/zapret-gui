...
> **Наш код их пока не знает.** В `WG_INTERFACE_FIELDS` (`core/awg_config.py`)
> этих ключей нет, поэтому импортированный AWG3-профиль: (а) получит от
> `validate()` пометку «неизвестное поле», (б) **потеряет их при
> `render_setconf`** — демон поднимется в режиме без header-protection, а пир,
> который её ждёт, будет дропать data-пакеты. Симптом — тот самый «92 B in /
> 20 KB out» (§12). Если такие профили пойдут в поддержку — добавлять поля в
> `WG_INTERFACE_FIELDS`, а `PersistentKeepalive` в `validate()` учить формату
> `range` (сейчас там строгий int, а AWG3 разрешает `22-30`).
...
В `WG_PEER_FIELDS` его у нас нет — при импорте чужого профиля он будет отброшен
как неизвестный. Практика показывает, что в клиентских конфигах Amnezia он
встречается редко, но при разборе «странного» профиля искать его стоит.
...
> ⚠️ **Наш код знает только старый путь.** `awg_platform.uapi_path()`,
> подчистка сокета в `awg_manager._kill_daemon()` и «Способ 3» поиска
> интерфейсов в `awg_detector` захардкожены на `/var/run/wireguard/`. С
> бинарями v3 это значит: сокет после `down` не удаляется (чистит его сам
> демон при штатном выходе — но не после SIGKILL), а обнаружение интерфейсов
> по сокетам не находит ничего. Оба места должны проверять **оба** каталога:
> на роутере вполне может стоять и старый `amneziawg-go`. На путь запуска это
> не влияет — `_do_up` сокет намеренно НЕ ждёт (был регресс, ронявший AWG на
> Keenetic, см. комментарий в `_do_up`).
...
3. **версия бинаря** `amneziawg-go --version` / `awg --version` — поддерживает
   ...
   разные симптомы, не перепутай. Отдельно: наш `render_setconf` вырежет
   AWG3-поля просто потому, что их нет в `WG_INTERFACE_FIELDS` (§2.1.1), —
   тогда в `awg showconf` их не будет даже на свежем бинаре.
