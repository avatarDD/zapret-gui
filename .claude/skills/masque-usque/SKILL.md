...
2. **[Diniboy1123/usque](https://github.com/Diniboy1123/usque)** — апстрим
   ...
   Актуальный релиз — **v4.2.1**; §3 остаётся валиден и для него, см. ниже.

   **v4.2.0 → v4.2.1** — ровно один коммит и ни одного изменения в CLI,
   флагах или формате `config.json`: в `api/masque.go` и `api/l4proxy.go`
   соединение поднимается через `quic.Transport{ConnectionIDLength: 20}`
   вместо голого `quic.Dial`. Без явной длины Connection ID бэкенд
   Cloudflare периодически отвечал `PROTOCOL_VIOLATION` и **рвал
   установленное соединение**. Практический симптом на 4.2.0: туннель
   поднимается, какое-то время работает, потом отваливается без внятной
   причины — если такое видите, проверьте версию бинарника
   (`usque version`) и обновите. Наша сборка тянет апстрим автоматически
   (job `auto-tag` в `build-usque-binaries.yml`), так что достаточно
   переустановить бинарник из GUI.
