...
## Как каталоги используются

* **Семейство трафика** приёма — по имени файла: `http80_*` → HTTP,
  `tcp_*` → TLS, `udp_*` → QUIC, `discord_voice_*`/`voice*` → голос
  (`catalog_loader.catalog_family`). Подбор берёт приёмы только своего
  семейства: HTTP-приём для TLS-цели — пустая проба. Новый файл называйте
  по этой схеме, иначе он попадёт во все подборы протокола.
* **Наборы подбора** (quick ~30, standard ~80, full — все) — по очереди из
  каждой техники приёмов (`technique_key`), без повторов по аргументам.
  `label = recommended` — лишь подсказка порядка внутри техники: ею
  помечена четверть каталога, отбирать по ней одной нельзя. В списке
  стратегий GUI метки `recommended`/`stable`/`experimental` не
  показываются; `caution` — показывается как «Осторожно», `game` — «Для
  игр».
* **Одинаковый `section_id` с разными аргументами** в разных файлах
  (голос и QUIC держат `fake_2_n2` с разными блобами) — первая запись
  сохраняет id, остальные получают `<id>__<хеш args>` и подпись
  «· QUIC»/«· голос Discord». Раньше вторая была не видна ни в подборе,
  ни в списке.
...
quick = cm.get_quick_set(protocol="tcp", family="tls")  # ~30, разные техники
