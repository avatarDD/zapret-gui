...
# Источник истины для дефолтных исключений. Должен совпадать с бандлами
# import/lists/netrogat.txt и netrogat.base.txt — это проверяет
# tests/test_netrogat_consistency.py. Объединяет прежний netrogat +
# list-exclude.txt + бизнес-сервисы (ЭДО/фискальные/госуслуги, issue #166).
DEFAULT_NETROGAT = [
    # Госуслуги / госсектор
    "gosuslugi.ru",
    "government.ru",
    ...
    # ЭДО / фискальные / госсервисы для бизнеса (issue #166)
    "taxcom.ru",
    "1c.ru",
    "1c-edo.ru",
    "1c-ofd.ru",
    ...
    "garant.ru",
    "moedelo.org",
    # Объединено из list-exclude.txt (issue #166)
    "twitch.tv",
    "ttvnw.net",
    "live-video.net",
    "donationalerts.com",
...
class HostlistManager:
    ...
    def import_from_url(self, name, url):
        ...
        try:
            ...
            # Проверка сертификата ВКЛЮЧЕНА: содержимое попадает в hostlist
            # обхода DPI, поэтому MITM-подмена доменов недопустима (раньше
            # здесь стоял CERT_NONE «для роутеров» — это была дыра).
