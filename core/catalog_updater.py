...
def _fetch_json(url: str) -> dict:
    ...
    try:
        ...
        if e.code == 404:
            # 404 здесь — не «нет сети» и не «не тот путь внутри дерева», а
            # пропавший источник: репозиторий переименован, закрыт или удалён
            # вместе с аккаунтом. Голое «HTTP 404» отправляло пользователя
            # чинить свой роутер, хотя чинить нужно адрес в настройках сборки.
            raise Exception(
                "Источник каталогов стратегий недоступен: GitHub отвечает "
                "404 на %s/%s. Похоже, репозиторий переехал или удалён — "
                "обновление каталогов работать не будет, пока адрес "
                "источника не поправят в сборке GUI."
                % (SOURCE_OWNER, SOURCE_REPO)
            )
