...
DNS_SERVERS = {
    ...
    # IP — из A-записи dns.comss.one. Прежний 9.9.9.10 — адрес Quad9 без
    # фильтрации: правило «через Comss» на деле слало запросы в Quad9.
    "comss": {"doh": "https://dns.comss.one/dns-query", "ip": "83.220.169.155"},
...
class DnsRoutingManager:
    ...
    def apply(self) -> dict:
        ...
        # Пустой список правил — тоже состояние, которое надо записать:
        # раньше apply() сразу выходил, и после удаления последнего
        # правила в dns-routing.conf оставались прежние server=… — dnsmasq
        # продолжал слать эти домены на старый сервер.
        if not rules and not os.path.isfile(self._conf_path()):
        ...
        # Записываем в файл
        try:
            dns_file = self._conf_path()
            from core.safe_io import atomic_write_text
            atomic_write_text(
                dns_file,
                "# Auto-generated by zapret-gui dns_routing\n"
                + "".join(line + "\n" for line in lines),
                mode=0o644)
            ...
            if main_conf:
                ...
                # OpenWrt: dnsmasq живёт в procd-ujail и видит только
                # прокинутые пути. Без addnmount он не прочитал бы
                # conf-file из /opt и не стартовал — а с ним DHCP и DNS.
                jail = dnsmasq.ensure_jail_mount(dns_file)
                if not jail.get("ok"):
                    return {"ok": False, "applied": 0, "file": dns_file,
                            "dnsmasq_found": True,
                            "error": "dnsmasq в ujail, а прокинуть в него "
                                     "%s не удалось: %s — include не "
                                     "добавлен, чтобы dnsmasq не упал"
                                     % (dns_file, jail.get("error", ""))}
    ...
    def _conf_path() -> str:
        ...
        base = get_config_manager().get("zapret", "base_path",
                                        default="/opt/zapret2")
        return os.path.join(base, "lists", "dns-routing.conf")
