...
class _IfaceBuffer:
    """Кольцевой буфер сэмплов для одного интерфейса."""

    __slots__ = ("samples", "last_idx")

    def __init__(self):
        # Список кортежей (ts: int, rx: int, tx: int).
        # Пустые слоты — None.
        self.samples = [None] * RAW_BUFFER_SIZE
        self.last_idx = -1

    def append(self, ts: int, rx: int, tx: int):
        idx = (self.last_idx + 1) % RAW_BUFFER_SIZE
    ...
    def iter_chronological(self):
        ...
        if self.last_idx < 0:
            return
        n = RAW_BUFFER_SIZE
        # Идём от позиции после последней (самой старой) к самой
        # последней.
...
def _series_from_buffer(buf: _IfaceBuffer, window_sec: int,
                       points: int) -> list:
    """
    ...
    Если в окне нет данных или была только одна точка, серия
    пустая.
    """
    raw = list(buf.iter_chronological())
    if len(raw) < 2:
        return []
    ...
    # Делим окно на bucket'ы и берём среднюю bps в каждом.
    # bps в bucket = (rx_end - rx_start) / (ts_end - ts_start)
    out = []
    ...
    for b in range(points):
        bucket = buckets[b]
        if len(bucket) < 2:
            # Не хватает данных для bps в этом bucket'е — пропускаем
            continue
        ...
        if dt <= 0:
            continue
        # Защита от сброса счётчика (рестарт туннеля): если delta < 0,
        # считаем bps как 0.
        drx = max(0, rx1 - rx0)
        dtx = max(0, tx1 - tx0)
        out.append({
...
class TrafficSampler:
    ...
    def __init__(self):
        ...
        self._buffers  = {}    # iface → _IfaceBuffer
        self._thread   = None
