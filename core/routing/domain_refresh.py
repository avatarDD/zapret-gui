...
def kick(delay_sec: float = 0):
    """
    Разбудить демона, немедленно или через delay_sec секунд.

    Отложенный вариант нужен после подъёма туннеля: apply-хук
    (core/routing/applier) резолвит домены сразу, но в этот момент
    туннель ещё не сделал handshake, а резолвер роутера может смотреть
    в него — резолв проваливается, set остаётся пустым, и маршрут
    «оживает» только на следующем такте демона (по умолчанию 10 минут).
    Пара отложенных пинков закрывает это окно за секунды.
    """
    ensure_started()
    if delay_sec <= 0:
        _wake.set()
        return
    t = threading.Timer(delay_sec, _wake.set)
    t.daemon = True
    t.start()
