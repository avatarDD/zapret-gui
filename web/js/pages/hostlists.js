...
const HostlistsPage = (() => {
    ...
    function render(container) {
                ...
                <div class="lists-content" id="hl-content">
                    <div class="lists-toolbar">
                        <div class="lists-toolbar-left">
                            ...
                            <span class="lists-search-status" id="hl-search-status"></span>
                        ...
                        <div class="lists-toolbar-right">
                            <div class="list-ui-search" style="flex:0 0 240px; min-width:160px; max-width:280px;">
                                ...
                                <input type="text" class="form-input list-ui-search-input" id="hl-search-input"
                                       placeholder="Поиск в списке (Enter — след., Shift+Enter — пред.)"
                                       spellcheck="false" autocomplete="off">
                                <button class="list-ui-search-clear" id="hl-search-clear" title="Очистить" style="display:none;">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12">
                                        <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                                    </svg>
                                </button>
                            </div>
                            <button class="btn btn-ghost btn-sm" onclick="HostlistsPage.sortDedupe()" title="Отсортировать и удалить дубликаты">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14">
        ...
        const searchClear = document.getElementById('hl-search-clear');
        if (searchInput) {
    ...
    function updateSearchStatus(q) {
        const status = document.getElementById('hl-search-status');
        const editor = document.getElementById('hl-editor');
    ...
    /**
     * Найти следующее/предыдущее вхождение q в textarea, проскроллить и выделить.
    ...
    function findInEditor(q, direction = 1) {
        ...
        if (direction > 0) {
            ...
            if (pos < 0) pos = hay.indexOf(needle); // wrap
        } else {
            const slice = hay.slice(0, Math.max(0, editor.selectionStart - 1));
            pos = slice.lastIndexOf(needle);
            if (pos < 0) pos = hay.lastIndexOf(needle); // wrap
        }
        if (pos < 0) return;

        editor.focus();
        editor.setSelectionRange(pos, pos + q.length);
        // Прокрутить к выделению (примерно по числу строк до позиции)
    ...
    /**
     * Отсортировать строки + удалить дубликаты, обновить textarea.
     * Помечает как несохранённое.
     */
    function sortDedupe() {
        const editor = document.getElementById('hl-editor');
