...
const Help = (() => {
    ...
    register('strategies', {
        ...
        body: 'Стратегия — набор аргументов nfqws2 для обхода DPI. ' +
              ...
              '<b>Подсказка о цели при сохранении.</b> Если в профиле есть приём, ' +
              'но не указан ни один критичный кусок цели — <code>--hostlist</code>/' +
              '<code>--hostlist-domains</code>/<code>--hostlist-auto</code>/' +
              '<code>--ipset</code>/<code>--ipset-ip</code> (домен/ip), — при ' +
              'сохранении появится подсказка с примерами (забытый кусок выделен). ' +
              'Без него десинк затронет весь трафик очереди. Можно вернуться и ' +
              'дописать вручную («+ hostlist…») или сохранить как есть — тогда ' +
