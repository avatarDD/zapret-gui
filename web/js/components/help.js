...
const Help = (() => {
    ...
    register('strategies', {
        ...
        body: 'Стратегия — набор аргументов nfqws2 для обхода DPI. ' +
              ...
              'через <code>nfqws2 --dry-run</code>: движок разбирает опции и ' +
              'проверяет наличие файлов (<code>--blob</code>/' +
              '<code>--lua-init</code>/<code>--hostlist</code>), НЕ поднимая ' +
              'NFQUEUE и не трогая трафик. Ловит кривой парсинг опций и ' +
              'отсутствующие файлы. <b>Lua при этом не загружается</b> — ' +
              'синтаксис lua-скриптов и наличие вызываемых <code>--lua-desync</code> ' +
              'функций dry-run НЕ проверяет (это закрыто тестами проекта).<br><br>' +
