...
### Исправлено
- **`awg setconf … timeout` теперь подсказывает про I1–I5**
  (`core/awg_manager.py::_apply_setconf`). Если setconf падает/виснет, а в
  отрендеренном конфиге есть signature-пакеты `I1..I5`, к сообщению об
  ошибке добавляется подсказка: для Cloudflare WARP (это ванильный
  WireGuard) `I1–I5` не нужны и могут вешать `amneziawg-go` — стоит убрать
  `I1`. Это самый частый «висяк» демона на импортированных WARP-конфигах с
  крупным `I1` (UAPI-запрос принят, ответа нет → наш сторож рубит по 15 с и
  откатывает интерфейс, оставляя пустую диагностику). Тесты —
  `test_awg_manager_lifecycle.py::TestSetconfIpacketHint`.
