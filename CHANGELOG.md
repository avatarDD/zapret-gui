...
### Добавлено
- **Подсказка о забытом куске цели при создании стратегии** — если в
  профиле есть приём (`--lua-desync`), но не указан ни один критичный
  кусок цели (`--hostlist`/`--hostlist-domains`/`--hostlist-auto`/
  `--ipset`/`--ipset-ip`, т.е. домен/ip), при сохранении показывается
  подсказка с примерами, где забытый кусок выделен. Сохранить «как есть»
  (с авто-ограничением, как раньше) можно только подтвердив — иначе можно
  вернуться и дописать вручную (`web/js/pages/strategies.js`).
- **Переключатель режима отладки nfqws2 в карточке активной стратегии** —
  «🐞 Отладка nfqws2» включает `--debug` (`nfqws.debug`): пер-пакетный
  лог в «Журнал» на уровне INFO. Если nfqws2 запущен — сразу
  перезапускается с отладкой. Дубль тумблера добавлен в «Настройки →
...
- **BlockCheck2: найденные стратегии появлялись только в конце.** Вывод
  штатного `blockcheck2.sh` шёл через обычный pipe — libc в скрипте и его
  детях (curl, nfqws2) переключалась на блочную буферизацию, и строки
  `working strategy found` доходили до GUI лишь по завершении всех тестов.
  Теперь вывод направляется в PTY (`isatty()==true` → построчная
  буферизация), читается из мастер-дескриптора построчно — рабочие
  стратегии появляются в списке кликабельных бейджей В РЕАЛЬНОМ ВРЕМЕНИ и
  доступны для создания сразу же (`core/blockcheck2.py`). При недоступности
  PTY — фолбэк на pipe.
