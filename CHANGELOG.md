...
- **BlockCheck2: бейджи найденных стратегий не появлялись в реальном
  времени.** Два слоя: (1) вывод штатного `blockcheck2.sh` направлен в PTY
  (`isatty()==true` → построчная буферизация вместо блочной у pipe),
  читается из мастер-дескриптора построчно (`core/blockcheck2.py`,
  фолбэк на pipe); (2) фронтенд теперь разбирает строки
  `working strategy found` **прямо из потокового лога** на клиенте
  (зеркало `parse_found_strategy`) и создаёт бейдж в момент появления
  строки — не дожидаясь серверного списка `found` и цикла опроса. Списки
  сливаются с дедупом, интервал опроса 1200→800 мс
