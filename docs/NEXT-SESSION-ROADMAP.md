...
**✅ Сделано (эта сессия).** Главный баг: кнопка-бургер жила ВНУТРИ сайдбара,
который на ≤768px уезжает за экран (`translateX(-100%)`) — меню на телефоне
было не открыть вовсе (CSS ссылался на несуществующий
`#sidebar-toggle-mobile`). Сделано: мобильная шапка `#mobile-topbar`
(index.html, первый ребёнок `#app`; на ≤768px `#app` становится колонкой) —
бургер + название текущего раздела («Родитель · Подраздел» для детей дерева,
обновляется в `Sidebar.setCurrentPage`); затемнение `#sidebar-backdrop`
(показывается чистым CSS `.sidebar.open + .sidebar-backdrop`, перехватывает
тап — клик «сквозь» не проходит); кнопка в шапке самого меню стала крестиком
«закрыть»; закрытие: backdrop / ✕ / Esc / выбор пункта; aria-expanded на
бургере. CSS (`style.css`): dvh-высоты (body/#app/.sidebar — на мобильных
100vh выше видимой области), `--sidebar-width: min(280px,85vw)` на ≤768;
таблицы `#page-container table, .modal-content table` на ≤768 —
`display:block; overflow-x:auto` (горизонтальный скролл вместо сжатия в кашу,
авто-раскладка вместо table-layout:fixed); ≤600px все инлайн-гриды форм
(`div[style*="grid-template-columns"]` — сознательный селектор по
style-атрибуту, иначе править каждую страницу) складываются в 1 колонку,
включая сплит «280px 1fr» в awg_configs (`#awg-cfg-list` ограничен 40vh);
модалки: backdrop-padding 10px, max-height 100dvh, resizable-окно редактора
стратегий (JS-минимум 420px > ширины телефонов) зажато
`max-width: calc(100vw-16px)`; тосты прижаты к низу (не закрывают бургер);
pointer:coarse расширен на голые input/select/textarea (font-size 16px — нет
автозума iOS, min-height 40px) и чекбоксы 18px; фикс прозрачного sticky
тулбара списков (`var(--bg)` не существовала → `--bg-primary`). Viewport:
убран `user-scalable=no` (pinch-zoom полезен на плотных таблицах, автозум
полей решён 16px-шрифтом). Проверено Playwright (Pixel 7 412px и 360px):
меню/навигация/Esc/backdrop работают, прокси-таблица и таблица маршрутов
скроллятся, формы складываются, страница не переполняется по горизонтали,
десктоп 1280px — без изменений.
